...
function retrieveMajor(semver) {
  return semver.slice(0, semver.indexOf('.'));
}

function retrieveMinor(semver) {
  return semver.slice(semver.indexOf('.') + 1, semver.indexOf('.', semver.indexOf('.') + 1));
}

function retrievePatch(semver) {
  return semver.slice(semver.indexOf('.', semver.indexOf('.') + 1) + 1);
}
