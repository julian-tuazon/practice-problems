...
function scaleTip(arr) {
  const middleIndex = arr.indexOf("I");
  const leftSum = arr.slice(0, middleIndex).reduce((acc, cur) => acc + cur, 0);
  const rightSum = arr.slice(middleIndex + 1, arr.length).reduce((acc, cur) => acc + cur, 0);
  if (leftSum > rightSum) return "left";
  if (leftSum < rightSum) return "right";
  return "balanced";
}
